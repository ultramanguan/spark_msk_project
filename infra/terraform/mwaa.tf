# MWAA (managed Airflow) for orchestrating the production pipeline. Entirely optional -- gated behind
# var.enable_mwaa (default false). The DAG itself is at airflow/dags/retail_lakehouse_pipeline.py, synced
# to dag_s3_path below by scripts/deploy_aws.sh (run manually; see RUNBOOK_AWS.md).
#
...
resource "aws_mwaa_environment" "this" {
  ...
  requirements_s3_path = aws_s3_object.airflow_requirements[0].key
  max_workers          = 2
...
output "mwaa_webserver_url" {
  value = var.enable_mwaa ? aws_mwaa_environment.this[0].webserver_url : null
}
